...
# Dataset
################
    dataset_default ='ratings_Movies_and_TV'
...
# device #
    ...
    parser.add_argument('--device_idx', type=str, default='3')
# optimizer #
...
################
    parser.add_argument('--experiment_dir', type=str, default='experiments_mean%s_dataset%s_sample%s_KL%s_regularization%s_temp%s_betas%s_betat%s_betast%s_betasst%s'%(mean_default,dataset_default,sample_type_default,KL_default,regularization_default,temprature_default,beta_s_default,beta_t_default,beta_st_default,alpha_sst_default))
    parser.add_argument('--experiment_description', type=str, default='test_alpha%s_mask_prop%s'%(alpha_default,mask_prob_default))
    ...
    alpha_sst_list = [0]
    beta_t_list = [0.04]
    alpha_list = [0.004]#0, 0.0001, 0.0002, 0.0005,
    beta_st_list = [0]  
    for beta_t in beta_t_list:
        ...
        for beta_st in beta_st_list:
            ...
            for alpha_sst in alpha_sst_list:
                ...
                args.experiment_dir = 'experiments_split%s_mean%s_dataset%s_sample%s_KL%s_regularization%s_temp%s_betas%s_betat%s_betast%s_betasst%s'%(split_default,mean_default,dataset_default,sample_type_default,KL_default,regularization_default,temprature_default,beta_s_default,beta_t,beta_st,alpha_sst)
            
      
                for alpha in alpha_list:
                    args.alpha = alpha 
                    args.experiment_description='test_alpha%s_mask_prop%s'%(alpha,mask_prob_default)
                    print('alpha:',alpha)
                    train()

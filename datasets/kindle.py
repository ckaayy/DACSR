...
class KinDataset(AbstractDataset):
    @classmethod
    def code(cls):
        return 'kindle'
    ...
    def all_raw_file_names(cls):
        return ['ratings_Kindle_Store_test.csv',
                'ratings_Kindle_Store_train.csv',
                'ratings_Kindle.csv'
		]
    ...
    def load_ratings_df(self):
        if self.args.user_split == 'by_timestamp':
            ...
            file_path = folder_path.joinpath('ratings_Kindle_Store_train.csv')
            df_source = pd.read_csv(file_path, sep=',', header=None)
            #print(df_source.head())
            
            folder_path = self._get_rawdata_folder_path()
            file_path = folder_path.joinpath('ratings_Kindle_Store_test.csv')
            ...
            folder_path = self._get_rawdata_folder_path()
            file_path = folder_path.joinpath('ratings_Kindle.csv')
            ...
            group_size = df.groupby('sid').size()
            group_items = group_size.index[group_size >= 10] 
            df = df[df['sid'].isin(group_items)]

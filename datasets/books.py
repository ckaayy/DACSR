...
class BOOKDataset(AbstractDataset):
    @classmethod
    def code(cls):
        return 'books'
    ...
    #     return df
    def all_raw_file_names(cls):
         return ['ratings_Books.csv']

    def load_ratings_df(self):
            folder_path = self._get_rawdata_folder_path()
            file_path = folder_path.joinpath('ratings_Books.csv')
            df = pd.read_csv(file_path, sep=',', header=None)
            df.columns = ['uid', 'sid','rating','timestamp']
    ...
    #    which_users = df_group[df_group.columns[1]] < unix_timestamp
            ...
            # filter out those items not in the items sets of source

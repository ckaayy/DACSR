...
################
    ...
    parser.add_argument("--static_weight",type=float,default=0.9,help="Weight on the static (SimCLR‐style) loss term")
...
################
    ...
    for ne_seed in test_negative_sampling_seed_list:
        ...
        ################m
        mask_list = [0.4] #[0.4, 0.5, 0.6, 0.7, 0.8]
        alpha_sst_list = [0.04] #[0,0.01,0.04,0.1]
        beta_t_list = [0.01] #[0.01,0.05,0.1,0.2]
        alpha_list = [0.2]#[0,0.01,0.03,0.05]
        beta_st_list = [0] 
        val_best_before = 0
        ...
        for beta_t in beta_t_list:
            ...
            for beta_st in beta_st_list:
                ...
                for alpha_sst in alpha_sst_list:
                    ...
                    args.experiment_dir = 'WM_2_%s_itemshift%s_usersplit%s_maxlen%s_dataset%s_sample%s_split%s_temp%s'%(args.weighted_mean,args.itemshift, args.user_split, args.max_target_len,dataset_default,sample_type_default,args.split,temprature_default)

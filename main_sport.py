...
def train():
    ...
    #         # feats_s, _  = model(seqs_s) # for cew trainer + sasrecW (sasrec baseline)
    ...
    #         # feats_t, _ = model(seqs_t) # for cew trainer and sasrecW (sasrec baseline)
    ...
    # plt.legend(); plt.title("sport_byrandom_dacsr+_t-SNE")
    # plt.savefig(os.path.join(export_root, 'sport_byrandom_dacsr+_t-SNE.png'), dpi=300)
    # plt.show()
...
################
    ...
    parser.add_argument("--static_weight",type=float,default=1.0,help="Weight on the static (SimCLR‐style) loss term")
...
################
    ...
    for ne_seed in test_negative_sampling_seed_list:
        ...
        ################m
        mask_list = [0.5]#[0.4, 0.5, 0.6, 0.7, 0.8]
        alpha_sst_list = [0.07] #[0, 0.01, 0.02, 0.04]
        beta_t_list = [0.01] #[0, 0.01, 0.02, 0.05, 0.1]
        alpha_list = [0.2] #[0, 0.01, 0.02, 0.05, 0.1]
        beta_st_list = [0] #[0, 0.01, 0.04, 0.05]#[0,0.04] 
        val_best_before = 0

...
def train():
    ...
    # plt.legend(); plt.title("beauty_byrandom_dacsr+_t-SNE")
    # plt.savefig(os.path.join(export_root, 'beauty_byrandom_dacsr+_t-SNE.png'), dpi=300)
    # plt.show()
...
################
    parser.add_argument('--model_code', type=str, default='SASRecT',choices=['SASRecT','SASRecT_1','SASRecW', 'SIGMA', 'linrec'])#, choices=MODELS.keys()
...
################
    ...
    for ne_seed in test_negative_sampling_seed_list:
        ...
        ################m
        mask_list = [0.6] #[0.6,0.7,0.8]
        alpha_sst_list = [0.02]
        beta_t_list = [0.01]#[0.04, 0.1]
        alpha_list = [0.2]#[0,0.01, 0.04, 0.1]#, 0.0001, 0.0002, 0.0005,  
